...
def main():
    ...
    datas = {
        "item_id": st.session_state.item_select["article_id"],
        "user_id": st.session_state.user_id,
        "related_req_id": st.session_state.req_id,
    }
    response = requests.post(os.environ["url"] + "/api/interact", json=datas)
    ...
    if response.status_code == 200:
        ...
        raise Exception("Error in recording interaction")
